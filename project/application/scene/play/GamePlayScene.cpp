...
{
    // 終了演出の開始。ここではフェードアウトに移る例とする。
    // 直接 Exit に遷移させ、Exit 側でフェード完了を待つ。
...
{
    // 必要なら終了演出中の監視処理を入れる（今回はすぐ Exit に遷移済み）
}
...
{
    // フェードが Start 済みであればそのまま待つ。未設定ならここで開始する。
    // （既に OnEnterEnd で Start している場合は何もしない）
}
...
void GamePlayScene::OnUpdateExit()
{
    ...
    {
        // フェード完了 → 結果画面などへ遷移
        if (sceneManager_) sceneManager_->ChangeScene("RESULT");
    }
}
