...
public:
	 ...
	 * @note 返却済みの番号は使わず、必ず末尾から連番で取る。GBuffer みたいに
	 *       続けて取った番号を1つの表として読む側があるので、ここで使い回すと表が崩れる。
	 */
	uint32_t Allocate();

	/**
	 * @brief 返却済みの番号があれば使い回して1つ確保する。
	 * @return 確保されたSRVインデックス
	 * @note 単体でしか参照しない SRV（読み込んだテクスチャなど）専用。表として並べて使うものには使わない。
	 */
	uint32_t AllocateReusable();
